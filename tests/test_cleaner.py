...
def test_db():
    with tempfile.NamedTemporaryFile() as f:
        ...
        cur.execute(
            """
        CREATE TABLE main.t1 (c1 TEXT PRIMARY KEY, c2 TEXT, c3 TEXT, c4 TEXT)
        """
        )
        cur.execute(
            """
        INSERT INTO main.t1 VALUES (1, 'A name', 'a-name', 'ABC')
        """
        )
        cur.execute(
            """
        INSERT INTO main.t1 VALUES (2, 'A name 2', 'a-name', 'ABC')
        """
        )
        cur.execute(
            """
        INSERT INTO main.t1 VALUES (3, 'A name 3', 'a-name', 'ABC')
        """
        )
        conn.commit()
        conn.close()
...
def test_clean_simple(piis):
    ...
    row = {"c1": "unmasked", "c2": "masked_name", "c3": "masked_with_arg", "c4": "CDE"}
